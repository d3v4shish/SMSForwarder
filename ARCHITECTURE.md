...
The View-based UI uses app-local resource tokens: warm off-white canvas (`#F3F0E8`), near-black primary text, sharp 3dp bordered surfaces, and small elevation. Blue denotes interaction, green healthy access, amber action required, red destructive actions, purple intelligent/derived output, and grey inactive state. Phone numbers use platform monospace; normal UI uses Android's Roboto-family system sans. These helpers stay in `MainActivity` until a second app needs a shared design-system module.
